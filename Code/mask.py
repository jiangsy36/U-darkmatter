...
def get_max_in_mask(mask):
    # 检查输入维度
    if len(mask.shape) != 4:
        mask = np.expand_dims(mask, axis=-1)  # 添加通道维度如果没有
        
    # 获取最大值的位置
    mask_max = np.argmax(mask, axis=-1)  # 在最后一个维度上取argmax
    return mask_max  # 输出形状: (batch_size, height, width)
...
def statistics(mask_max):
    ...
    class_names = ['Background', 'Glowing', 'Hot pixel', 'Cluster']
    for i, name in enumerate(class_names):
        if i < len(class_frequency):
            print('> {0}: {1} %'.format(name, round(class_frequency[i]*100, 2)))
        else:
